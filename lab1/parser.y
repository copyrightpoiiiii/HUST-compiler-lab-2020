...
//  %type 定义非终结符的语义值类型
%type  <ptr> program ExtDefList ExtDef  Specifier ExtDecList FuncDec CompSt VarList VarDec ParamDec Stmt StmList DefList Def DecList Dec Exp Args STRUCTNAME VAR1 CONDI CALC FORExp CLACList//STRUCT_SPECIFER
...
%token EXT_DEF_LIST EXT_VAR_DEF FUNC_DEF FUNC_DEC EXT_DEC_LIST PARAM_LIST STRUCT_DEF PARAM_DEC VAR_DEF DEC_LIST DEF_LIST COMP_STM STM_LIST EXP_STMT IF_THEN IF_THEN_ELSE
%token FUNC_CALL ARGS FUNCTION PARAM ARG ARRAY_CALL STRUCT_CALL VAR_CALL BREAK CONTINUE
...
ExtDecList:  VarDec      {$$=$1;}      
           ...
           ;  
/*STRUCT_SPECIFER: STRUCT STRUCTNAME LC ExtDefList RC {
    $$=mknode(2,STRUCT_DEF,yylineno,$2,$4);
}| STRUCT ID {
    $$=mknode(1,STRUCT_DEF,yylineno,$2);}
;*/
...
Stmt:   Exp SEMI    {$$=mknode(1,EXP_STMT,yylineno,$1);}
      ...
      ;
FORExp: VAR1 SEMI CONDI SEMI CALC {
    $$ = mknode(3,FOREXP,yylineno,$1,$3,$5);
}
;
VAR1: {$$=NULL;}|VarDec ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}
;
CONDI: {$$=NULL;}|VarDec RELOP Exp{$$=mknode(2,RELOP,yylineno,$1,$3);strcpy($$->type_id,$2);}
;
CALC:  {$$=NULL;}
      | CLACList {$$=$1;}
      ;
...
Dec:     VarDec  {$$=$1;}
       ...
       ;
Exp:    VarDec ASSIGNOP Exp {$$=mknode(2,ASSIGNOP,yylineno,$1,$3);strcpy($$->type_id,"ASSIGNOP");}
      ...
      | Exp DOT Exp    {$$=mknode(2,STRUCT_CALL,yylineno,$1,$3);strcpy($$->type_id,"DOT");}
      | LP Exp RP     {$$=$2;}
      | MINUS Exp %prec UMINUS   {$$=mknode(1,UMINUS,yylineno,$2);strcpy($$->type_id,"UMINUS");}
      | NOT Exp       {$$=mknode(1,NOT,yylineno,$2);strcpy($$->type_id,"NOT");}
      | DPLUS  VarDec    {$$=mknode(2,DPLUS,yylineno,NULL,$2);strcpy($$->type_id,"DPLUS");}
      | VarDec DPLUS      {$$=mknode(1,DPLUS,yylineno,$1);strcpy($$->type_id,"DPLUS");}
      | DMINUS  VarDec     {$$=mknode(2,DMINUS,yylineno,NULL,$2);strcpy($$->type_id,"DMINUS");}
      | VarDec DMINUS      {$$=mknode(1,DMINUS,yylineno,$1);strcpy($$->type_id,"DMINUS");}
      | VarDec ADDASSIGN Exp {$$=mknode(2,ADDASSIGN,yylineno,$1,$3);strcpy($$->type_id,"ADDASSIGN");}
      | VarDec MINUSASSIGN Exp {$$=mknode(2,MINUSASSIGN,yylineno,$1,$3);strcpy($$->type_id,"MINUSASSIGN");}
      | VarDec STARASSIGN Exp {$$=mknode(2,STARASSIGN,yylineno,$1,$3);strcpy($$->type_id,"STARASSIGN");}
      | VarDec DIVASSIGN Exp {$$=mknode(2,DIVASSIGN,yylineno,$1,$3);strcpy($$->type_id,"DIVASSIGN");}
