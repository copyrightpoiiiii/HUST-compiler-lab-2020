...
void prnIR(struct codenode *head){
    ...
    do {
        ...
        //printf("%d %s\n",h->op,opnstr1);
        switch (h->op) {
            case ADDASSIGN: printf("  %s := %s + (%s)\n",resultstr,resultstr,opnstr1);
                            break;
            case MINUSASSIGN: printf("  %s := %s - (%s)\n",resultstr,resultstr,opnstr1);
                            break;
            case STARASSIGN: printf("  %s := %s * (%s)\n",resultstr,resultstr,opnstr1);
                            break;
            case DIVASSIGN: printf("  %s := %s \\ (%s)\n",resultstr,resultstr,opnstr1);
                            break;
            ...
            case ARRAY_CALL:
                            printf("  %s",resultstr);
                            struct ASTNode * T0 = h->result.aC;
                            while(T0->kind == ARRAY_CALL){
                                struct ASTNode *T1=T0->ptr[1];
                                if(T1->kind == ID)printf("[%s]",T1->type_id);
                                else if(T1->type_int>=0) printf("[%d]",T1->type_int);
                                T0 = T0->ptr[0];
                            }
                            printf(":= %s\n",opnstr1);
...
void prnArray(struct arrayInfo *T){
    ...
    {
        printf("%6d ",T->sizeInfo[i]);
    }
    printf("\n");
}
void prn_symbol(){ //显示符号表
    int i=0;
    printf("%6s %10s %10s %9s %6s %10s\n","变量名","别 名","层 号","类  型","标记","偏移量");
    for(i=0;i<symbolTable.index;i++){
        ...
        if(symbolTable.symbols[i].type==INT)
            ...
            printf("%6s ","char");
        else printf("%6s ","struct");
        printf("%4c %7d \n",symbolTable.symbols[i].flag,symbolTable.symbols[i].offset);
        if(symbolTable.symbols[i].flag=='A')
            prnArray(symbolTable.symbols[i].arrInfo);
        if(symbolTable.symbols[i].type==STRUCT){
            int ind =symbolTable.symbols[i].strInfo;
            printf("结构体名称为: %6s \n",structTable[ind].name);
            printf("定义: %6s %6s %6s\n","变量名","类型","标记");
            for(int j=0;j<structTable[ind].index;j++){
                printf("%6s ",structTable[ind].varList[j].name);
                if(structTable[ind].varList[j].type==INT)
                    printf("%6s ","int");
                else if(structTable[ind].varList[j].type==FLOAT)
                    printf("%6s ","float");
                else if(structTable[ind].varList[j].type==CHAR)
                    printf("%6s ","char");
                else printf("%6s ","struct");
                printf("%4c\n",structTable[ind].varList[j].flag);
            }
        }
            ;
    }
...
int fillSymbolTable(char *name,char *alias,int level,int type,char flag,int offset) {
    ...
    if(!fillStruct){
        ...
        if(sFlag)
            symbolTable.symbols[symbolTable.index].strInfo=strIndex;
        ...
        /*符号查重，考虑外部变量声明前有函数定义，
        其形参名还在符号表中，这时的外部变量与前函数的形参重名是允许的*/
        for(i=structTable[strSize].index-1; i>=0 ; i--) {
            if (!strcmp(symbolTable.symbols[i].name, name))  return -1;
        }
        //填写符号表内容
        int ind = structTable[strSize].index;
        strcpy(structTable[strSize].varList[ind].name,name);
        strcpy(structTable[strSize].varList[ind].alias,"");
        //structTable[strSize].varList[ind].level=level;
        structTable[strSize].varList[ind].type=type;
        structTable[strSize].varList[ind].flag=flag;
        structTable[strSize].varList[ind].offset=offset;
        return structTable[strSize].index++;
    }
}

int fillStructTable(char *name,struct ASTNode* T){
    for(int i=strSize-1; i>=0 ; i--) {
        if (!strcmp(structTable[i].name, name))  return -1;
    }
    strcpy(structTable[strSize].name, name);
    structTable[strSize].index=0;
    fillStruct=1;
    semantic_Analysis(T);
    fillStruct=0;
    return strSize++;
...
int searchStructTable(char *name) {
    int i,flag=0;
    for(i=strSize-1;i>=0;i--){
        if (!strcmp(structTable[i].name, name))  return i;
    }
    return -1;
}
...
struct arrayInfo *arrayHandle(struct ASTNode *T){
    struct arrayInfo *newArr = (struct arrayInfo *)malloc(sizeof(struct arrayInfo));
    struct ASTNode *T0 = T;
    newArr->type = T->type;
    while(T0->kind == ARRAY){
        if (T0->size <= 0){
            semantic_error(T0->pos, "", "数组大小只能大于0");
            return NULL;
        }
        newArr->sizeInfo[newArr->size] = T0->size;
        newArr->size++;
        if(newArr->size > 10){
            semantic_error(T0->pos,"","数组维数过大");
            return NULL;
        }
        T0 = T0->ptr[0];
    }
...
void ext_var_list(struct ASTNode *T){  //处理变量列表
    ...
    switch (T->kind){
        ...
        case ARRAY:
            tmp = T;
            while(tmp->kind != ID){
                if(tmp->size)size *= tmp->size;
                tmp=tmp->ptr[0];
            }
            rtn = fillSymbolTable(tmp->type_id, newAlias(), LEV, T->type, 'A', T->offset+T->width*size);
            if (rtn == -1)
                semantic_error(T->pos, T->type_id, "变量名重复定义");
            else {
                struct arrayInfo *newArr = arrayHandle(T);
                if(newArr == NULL){
                                //throw("array error!");
                    break;
                }
                T->place = rtn;
                if(!fillStruct)
                    symbolTable.symbols[rtn].arrInfo=newArr;
                else structTable[strSize].varList[rtn].arrInfo=newArr;
            }
            break;
        }
}
...
{//处理基本表达式，参考文献[2]p82的思想
  ...
  if (T)
    ...
    case ARRAY:
                ...
                break;
    case INT:   
                T->place=fill_Temp(newTemp(),LEV,INT,'T',T->offset); //为整常量生成一个临时变量
                T->type=INT;
                ...
                break;
    case FLOAT: 
                T->place=fill_Temp(newTemp(),LEV,FLOAT,'T',T->offset);   //为浮点常量生成一个临时变量
                T->type=FLOAT;
                ...
                break;
    case CHAR:
                T->place = fill_Temp(newTemp(),LEV,CHAR,'T',T->offset);
                T->type = CHAR;
                opn1.kind = CHAR;
    ...
    case DIVASSIGN:
                ...
                break;
	case ASSIGNOP:
                if (T->ptr[0]->kind!=ID && T->ptr[0]->kind != ARRAY_CALL && T->ptr[0]->kind != ARRAY && T->ptr[0]->kind != STRUCT_CALL) semantic_error(T->pos,"", "赋值语句需要左值");
                else {
                    Exp(T->ptr[0]);   //处理左值，例中仅为变量
                    T->ptr[1]->offset=T->offset;
                    //printf("%s ",T->ptr[0]->type_id);
                    Exp(T->ptr[1]);
                    //printf("%s %d\n",T->ptr[1]->type_id,T->ptr[1]->kind);
                    ...
                    result.offset=symbolTable.symbols[T->ptr[0]->place].offset;
                    result.aC = T->ptr[0];
                    if(T->ptr[0]->kind!=ARRAY_CALL)T->code=merge(2,T->code,genIR(ASSIGNOP,opn1,opn2,result));
                    else T->code=merge(2,T->code,genIR(ARRAY_CALL,opn1,opn2,result));
                }
                break;
	...
	case DIV:   
                ...
                if (T->ptr[0]->type==FLOAT && T->ptr[1]->type==FLOAT)
                     T->type=FLOAT,T->width=T->ptr[0]->width+T->ptr[1]->width+8;
                else if (T->ptr[0]->type == INT && T->ptr[1]->type == INT)
                ...
                result.offset=symbolTable.symbols[T->place].offset;
                T->code=merge(3,T->ptr[0]->code,T->ptr[1]->code,genIR(T->kind,opn1,opn2,result));
                T->width=T->ptr[0]->offset+T->ptr[1]->width+(T->type==INT?4:8);
                if(T->type==CHAR)
                    T->width=T->ptr[0]->offset+T->ptr[1]->width+1;
                break;
	case NOT:   //未写完整
                T->ptr[0]->offset=T->ptr[1]->offset=T->offset;
                Exp(T->ptr[0]);
    ...
    case DMINUS:
                ...
                T->place = fill_Temp(newTemp(), LEV, T->type, 'T', T->offset + T0->width);
                opn1.kind = ID;
                ...
                result.type = T->type;
                result.offset = symbolTable.symbols[T->place].offset;
                T->code = merge(2, T0->code, genIR(T->kind, opn1, opn2, result));
                T->width = T0->width + 4;
                break;
    ...
    case ARRAY_CALL:
                ...
                T->ptr[0]->type = T->type = symbolTable.symbols[rtn].type;
                width=T->type==INT?4:8;   //存放函数返回值的单数字节数
                if(T->type == CHAR)
                    width = 1; 
                T->place = rtn;
                //Exp(T->ptr[1]); //处理所有实参表达式求值，及类型
                T0 = T;
                int dimSize = symbolTable.symbols[rtn].arrInfo->size,size=0,flag=1,off = 1;
                while(T0->kind == ARRAY_CALL){
                        struct ASTNode *T1=T0->ptr[1];
                        Exp(T1);
                        if(T1->type_int>=0 && T0->ptr[0]->kind == ARRAY_CALL) off*=T1->type_int;
                        else if(T1->type_int>=0) off+=T1->type_int;
                        if (T1->type != INT){
                            semantic_error(T->pos, T1->type_id, "数组下标非整型");
                            flag=0;
                            break;
                        }
                        else if (T1 -> type_int < 0){
                            semantic_error(T->pos, "", "数组下标为负数");
                            flag=0;
                            break;
                        }
                        size++;
                ...
                T->ptr[0]->offset=T->offset = symbolTable.symbols[rtn].offset+off*width;
                if(size < dimSize&&flag)semantic_error(T->pos,"","数组维数过小");
                break;
    case STRUCT_CALL:
                Exp(T->ptr[0]);
                //Exp(T->ptr[1]);
                ...
                while(T0->kind != ID)
                    T0 = T0->ptr[0];
                int i=0,ind = symbolTable.symbols[rtn].strInfo;
                for(;i<structTable[ind].index;i++){
                    if(!strcmp(T0->type_id,structTable[ind].varList[i].name))
                        break;
                }
                if(i >= structTable[ind].index){
                    semantic_error(T->pos,T0->type_id,"内部变量不存在");
                    break;
                }
                if(T->ptr[1]->kind == ARRAY_CALL){
                    T0 = T->ptr[1];
                    int dimSize = structTable[ind].varList[i].arrInfo->size,size=0,flag=1;
                    while(T0->kind == ARRAY_CALL){
                            struct ASTNode *T1=T0->ptr[1];
                            Exp(T1);
                            if (T1->type != INT){
                                semantic_error(T->pos, T1->type_id, "内部数组下标非整型");
                                flag=0;
                                break;
                            }
                            else if (T1 -> type_int < 0){
                                semantic_error(T->pos, "", "内部数组下标为负数");
                                flag=0;
                                break;
                            }
                            size++;
                            if(size > dimSize){
                                semantic_error(T->pos,"","内部数组维数过大");
                                flag=0;
                                break;
                            }
                            T0 = T0->ptr[0];
                    }
                    if(size < dimSize&&flag)semantic_error(T->pos,"","内部数组维数过小");
                }
                T->type = structTable[ind].varList[i].type;
                break;
    case BREAK:
                if (T->canJump != 1){
...
{//对抽象语法树的先根遍历,按display的控制结构修改完成符号表管理和语义检查和TAC生成（语句部分）
  ...
  if (T)
	...
	case EXT_VAR_DEF:   //处理外部说明,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
            ...
            else if(T->ptr[0]->type == STRUCT){
                //printf("%s\n",T->ptr[0]->ptr[0]->type_id);
                //strcpy(T->ptr[1]->type_id,T->ptr[0]->ptr[0]->type_id);
                T->type = T->ptr[1]->type = STRUCT;
                strIndex = fillStructTable(T->ptr[0]->ptr[0]->type_id,T->ptr[0]->ptr[1]);
                sFlag = 1;
                strcpy(T->ptr[0]->type_id,T->ptr[0]->ptr[0]->type_id);
                //break;
            }
            T->ptr[1]->offset=T->offset;        //这个外部变量的偏移量向下传递
            T->ptr[1]->width=T->type==INT?4:8;  //将一个变量的宽度向下传递
            if(T->type == CHAR)
                T->ptr[1]->width=T->type = 1;
	...
	case  PARAM_DEC:
            if(T->ptr[0]->type == STRUCT){
                srtn = searchStructTable(T->ptr[0]->type_id);
                if(srtn == -1){
                    semantic_error(T->ptr[1]->pos,T->ptr[0]->type_id, "未定义的结构体");
                    break;
                }
                sFlag=1;
                strIndex = srtn;
    ...
    case VAR_DEF://处理一个局部变量定义,将第一个孩子(TYPE结点)中的类型送到第二个孩子的类型域
                ...
                while (T0) {  //处理所以DEC_LIST结点
                    ...
                    if (T0->ptr[0]->kind==ID){
                        ...
                        int size = 1;
                        struct ASTNode *tmp = T0->ptr[0];
                        while(tmp->kind != ID){
                            if(tmp->size)size *= tmp->size;
                            tmp=tmp->ptr[0];
                        }
                        rtn = fillSymbolTable(tmp->type_id, newAlias(), LEV, T0->ptr[0]->type, 'A', T->offset+T->width*size);
                        if (rtn == -1)
                            ...
                            struct arrayInfo *newArr = arrayHandle(T0->ptr[0]);
                            if(newArr == NULL){
                                //throw("array error!");
                                break;
                            }
    ...
    case FOR:
                ...
                T->width=T->ptr[1]->width;
                T->code = merge(8, T0->ptr[0]->code, genLabel(T0->ptr[2]->Snext), \
					T0->ptr[1]->code, genLabel(T0->ptr[1]->Etrue), T->ptr[1]->code, \
					genLabel(T->ptr[1]->Snext),T0->ptr[2]->code, genGoto(T0->ptr[2]->Snext));
                T->ptr[1]->canJump = 0;
               /*
